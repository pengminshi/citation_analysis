...
roc_single <- function(pedicted_values,binary_truth,name = "mouse", fuller = F){
    # scales the predicted values (either regression or probability to 0-1 then 
    # creates ROC curve)
    min_prob <- min(pedicted_values)
    prob_values0 <- pedicted_values-min_prob # all positive
    max_prob <- max(prob_values0)
    prob_values <- prob_values0/max_prob # bounded by 1
    
    pred = prediction(predictions = prob_values,
                      labels = binary_truth)
    pref = performance(pred,measure="tpr", x.measure="fpr")
    
    if(fuller == T){
        data_plot = data.frame(x = pref@x.values[[1]],y= pref@y.values[[1]],
                               threshold = min_prob + max_prob* pref@alpha.values[[1]],
                               mouse = name)
    }else{
        data_plot = data.frame(x = pref@x.values[[1]],y= pref@y.values[[1]],
                               mouse = name)
...
smoothing_nyear <- function(citematrix, ns){ # ns should be an odd number
    m = nrow(citematrix)
    n = ncol(citematrix)
    l = (ns - 1)/2
    smooth_citematrix = matrix(0,nrow = m, ncol = n)
    
    for(j in 1:n){
        preind = max(j-l, 1)
        postind = min(j+l, n)
        smooth_citematrix[,j] = rowMeans(citematrix[,preind:postind])
    }
    return(smooth_citematrix)
}

# increasing_rate <- function(citesmooth){
#     m = nrow(citesmooth)
#     n = ncol(citesmooth)
#     
#     return((citesmooth[,2:n]-citesmooth[,1:(n-1)])/(citesmooth[,1:(n-1)]+1))
# }
...
trim_text_bigram <- function(docs,n){
    ...
    matrix <- TermDocumentMatrix(corpus, control=list(tokenize=BigramTokenizer))
...
#     # accuracy_long <- melt( accuracy, "data", "accuracy", -1 )
#     accuracy_long <- melt( accuracy, variable.name = "data",value.names = "accuracy",id.vars = "cutoff")
#     # accuracy_long[,1] = as.numeric(accuracy_long[,1]$cutoff)
#     # accuracy_long[,1] = as.numeric(accuracy_long[,3]$value)
#     plot <- ggplot( accuracy_long, aes( cutoff, value, group = data, color = data ) ) + 
